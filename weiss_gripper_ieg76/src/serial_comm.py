...
class SerialPortComm(threading.Thread):
	...
	def set_opening_speed(self, opening_speed=20):
                # Annahme: Param Nr. 4 = Öffnungsgeschwindigkeit
                grasp_index = 96
                param_nr = 4  
                opening_speed_hex = hex(opening_speed)[2:].zfill(2).upper()
                send_cmd = f"SETPARAM({grasp_index}, {param_nr}, [{opening_speed_hex}])\n"
                fmt = '>' + 'B' * len(send_cmd)
                bytelist = [ord(c) for c in send_cmd]
                payload = struct.pack(fmt, *bytelist)
                with self.serial_write_sync:
                        self.serial.write(payload)
                        rospy.loginfo(f"Opening speed set to {opening_speed}%")
                return True
