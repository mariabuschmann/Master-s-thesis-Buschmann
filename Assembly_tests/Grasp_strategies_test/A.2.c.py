#!/usr/bin/env python3
"""
Main controller for peg-in-hole with sensor-based grasp evaluation (direct correction) with spiral insertion
"""
...
class GripperClient:
    ...
    def open(self): return self.command(False)
    def close(self): return self.command(True)
...
class MainController(Node):
    ...
    def target_hole(self, correction=0.0):
        base = np.array([0.3659, 0.3505, 0.110])
        return base + np.array([self.hole["center_x"] + correction, self.hole["center_y"], 0.0])

    def spiral_insert(self, correction=0.0):
        down = self.target_hole(correction).copy()
        down[2] += 0.0025
        self.peginsertion.leadin_spiral_without_sensor(down)
        self.gripper.open()
        print("✅ Peg inserted with spiral.")
        return time.time()
    ...
    def run_flow(self):
        ...
        # Pick sequence
        pick = np.array([0.4065, 0.377, 0.124])
        ...
        end_time = self.spiral_insert(correction)
